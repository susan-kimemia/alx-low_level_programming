...
/**
 * main - copying one file to the other
 * @argc: arg count
 * @argv: arg vector
 *
 * Return: 1 (success), otherwise exit
 */
int main(int argc, char *argv[])
{
int src, dest, _read = 1024, wrete, closesrc, closedest;
...
while (_read == 1024)
{
_read = read(src, buf, sizeof(buf));
if (_read == -1)
check_IO_stat(-1, -1, argv[1], 'O');
wrete = write(dest, buf, _read);
if (wrete == -1)
